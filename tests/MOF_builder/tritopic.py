...
from functions.place_bbs import scaled_node_and_edge_vectors,place_edges,place_nodes_tri
...
class MOF_tri:
	def __init__(self,templates_dir,nodes_dir,edges_dir,template,node_topics):
		...
		self.linker_topics = 3 #tritopic class 
	...
	def load(self,WRITE_CIF):
		...
		for net in ct2g(template,templates_dir):
				...
				for va in vas:
					...
					for ea in eas:
						...
						placed_nodes, tri_node, frame_nbb_node,tri_node_name,node_bonds = place_nodes_tri(nvecs, nodes_dir)
		...
		self.tri_node_name = tri_node_name
	...
	def basic_supercell(self,supercell,term_file = 'data/methyl.pdb',boundary_cut_buffer = 0.00,edge_center_check_buffer = 0.0,cutxyz=[True,True,True]):
		...
		tritopic_edges_fcoords = merge_multitopic_node_edge_fc(TG,sc_unit_cell,tri_node_name,placed_nodes_fc,placed_edges_fc)		
	...
	def write_basic_supercell(self,gro,xyz):
		tempgro('30test.gro',self.all_connected_node_edge_cc[self.all_connected_node_edge_cc[:,5]==1])
