...
def get_mapping_between_nometal_linker_xyz(linker_topic,center_frag_nodes_num,center_Xs,single_frag_nodes_num,frag_Xs,template_xyz, new_xyz='Residues/EDGE.xyz'):
    ...
    if linker_topic == 2:
        ...
        frag1_Xs = [i+center_nums for i in frag_Xs] #frag_Xs is Xs indices in a single outer_edge_frag
        ...
        if linker_topic ==3:
            ...
            left_xs = list(set(frag1_Xs+frag2_Xs+frag3_Xs)-set(i[1]for i in cn_bond))
            ...
            for x_i in center_Xs:
                ...
                for x_j in frag1_Xs+frag2_Xs+frag3_Xs+frag4_Xs:
                    ...
                    if np.linalg.norm(x_center-x_frag) < 4:
                        cn_bond.append((x_i,x_j))
            left_xs = list(set(frag1_Xs+frag2_Xs+frag3_Xs+frag4_Xs)-set(i[1]for i in cn_bond))
            print(left_xs,'left_xs')   
            for m in range(len(labels)-4*3,len(labels)):
                if labels[m]=='C':
                    ...
                    for n in left_xs:
                        ...
                        if np.linalg.norm(x_frag-x_ooc) <5:
                            cn_bond.append((n,m))
                            print('np.linalg.norm(x_frag-x_ooc)',n,m,np.linalg.norm(x_frag-x_ooc))
