...
from functions.place_bbs import scaled_node_and_edge_vectors, place_edges, place_nodes_tetra
...
from functions.write_cifs import write_cif_nobond, merge_catenated_cifs
...
from functions.output import tempgro, temp_xyz, viewgro
...
SINGLE_METAL_MOFS_ONLY = False#configuration.SINGLE_METAL_MOFS_ONLY
...
class MOF_tetra:
	def __init__(self, templates_dir, nodes_dir, edges_dir, template, node_topics):
		# Initialize the MOF_tetra class with directories and template information
	...
	def load(self, WRITE_CIF):
		# Load the template and generate the graph
	...
	def defect_missing(self,remove_node_list = [],remove_edge_list = []):
		...
		if len(eG_subparts) > 1:
			print(f'this MOF has {len(eG_subparts)} separated fragments: {eG_subparts}')
		else:
