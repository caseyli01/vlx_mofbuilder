...
def nodepdb2G(pdbfile,metal):
    ...
    G = nx.Graph()
    for n in node_data:
        G.add_node(n[0],ccoords=(np.asarray([float(n[2]),float(n[3]),float(n[4])])),type=n[1])
    ...
    for i in range(len(node_data)):
        for j in range(i,len(node_data)):
            if con_matrix[i,j] == 1:
                if nn(node_data[i][0]) == nn(node_data[j][0]) and nn(node_data[i][0]) == metal: #skip same atom type bond metal-metal
                    continue
                else:
