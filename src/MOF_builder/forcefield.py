...
import re
import shutil
from map_forcefield import map_forcefield_by_xyz, parseff
from write_mapping_between import (
    write_mapping_file,
    get_mapping_between_nometal_linker_xyz,
)
from _output import temp_xyz, temp_xyz_nm
    ...
    for res in restypes:
        ...
        if res in ["METAL", "HHO", "HO", "O"]:
            ...
            if res in ["EDG", "EDGE"]:
                ...
                map_name = "linker_ff_mapping"
                parsed_path = parseff(ff_name)
                map_path = write_mapping_file(
                    parsed_path, mapping, metals, mol_metals, map_name
                )
                map_forcefield_by_xyz(
                    parsed_path, map_path, linker_file, new_xyz="EDG.xyz"
                )
