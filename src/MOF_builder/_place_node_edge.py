...
def place_edgeinnodeframe(
    ...
    linker_x_vecs,
    linker_ccoords,
    scaled_rotated_Xatoms_positions,
    scaled_rotated_chain_node_positions,
):
    ...
    for (i, j), pair in optimized_pair.items():
        ...
        norm_xx_vector = np.round(xx_vector, 8)
        ...
        placed_edge_ccoords = (
            np.dot(translated_linker_coords, rot) + x_i_x_j_middle_point
        )
