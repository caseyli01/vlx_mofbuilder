...
def cluster_analysis_bridging_node(
    array_points, unit_cell, cluster_size, distance_threshold
):
    fcoords = c2f_coords(array_points, unit_cell)

    # sep cell_points and cell_out points
    cell_in_points = [
        array_points[ind] for ind, i in enumerate(fcoords) if check_inside_unit_cell(i)
    ]
    cell_out_points = [
        array_points[ind]
        for ind, i in enumerate(fcoords)
        if not check_inside_unit_cell(i)
    ]
...
def extract_bridge_point_cluster_center_from_templatecif(
    cif_file, target_type, cluster_size, cluster_distance_threshhold
):
    cell_info, array_atom, array_target_atoms = (
        extract_type_atoms_fcoords_in_primitive_cell(cif_file, target_type)
    )
    unit_cell = extract_unit_cell(cell_info)
    # unit_cell = np.round(unit_cell,3)
    metal333 = make_supercell_3x3x3(array_target_atoms)
    ...
    cluster_centers_ccoords = cluster_analysis_bridging_node(
        array_metal_ccords, unit_cell, cluster_size, cluster_distance_threshhold
    )
...
def extract_cluster_center_from_templatecif(
    cif_file, target_type, cluster_size, cluster_distance_threshhold
):
    cell_info, array_atom, array_target_atoms = (
        extract_type_atoms_fcoords_in_primitive_cell(cif_file, target_type)
    ...
    # cluster analysis in cartesian coordinates
    array_metal_ccords = np.dot(unit_cell, metal333.T).T
    cluster_centers_ccoords = clust_analysis_points(
        array_metal_ccords, cluster_distance_threshhold
    )
    cluster_centers_ccoords = np.vstack(cluster_centers_ccoords)
    # cluster_centers should return fractional coordinates
    cluster_centers_fcoords = np.dot(
...
if __name__ == "__main__":
    ...
    vvnode, cell_info, unit_cell = extract_cluster_center_from_templatecif(
        template_cif_file, "YY", 1
    )  # node com in template cif file, use fcluster to find cluster and the center of the cluster
    eenode, _, _ = extract_cluster_center_from_templatecif(
        template_cif_file, "XX", 1
    )  # edge com in template cif file, use fcluster to find the cluster and center of the cluster
