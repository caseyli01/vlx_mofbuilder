import numpy as np
import re
from _isolated_node_cleaner import filter_connected_node_loose
...
def check_edgex_sits_inboundary(pointx1, pointx2, box_bound):
    ...
    doublecheck = check_edge_center_inbox(
        pointx1, box_bound
    ) and check_edge_center_inbox(pointx2, box_bound)
    return doublecheck
...
def filt_edgex_fvec(array):
    edgex_fvec = np.asarray(
        [i for i in array if i[4] == "EDGE" and re.sub("[0-9]", "", i[2]) == "X"]
    )
    return edgex_fvec
...
def filt_edgex_fvec_tetra(array):
    edgex_fvec = np.asarray(
        [i for i in array if i[4] == "TEDGE" and re.sub("[0-9]", "", i[2]) == "X"]
    )
    return edgex_fvec
...
def cut_boundary(arr, box_bound, buffer, cutx, cuty, cutz):
    ...
    for i in range(len(arr)):
        if cutx:
            new_arr[i][0] = limit_x_range(
                arr[i][0], box_bound_xmin, box_bound_xmax, box_bound_xl
            )
        else:
            new_arr[i][0] = arr[i][0]
        if cuty:
            new_arr[i][1] = limit_x_range(
                arr[i][1], box_bound_ymin, box_bound_ymax, box_bound_yl
            )
        else:
            new_arr[i][1] = arr[i][1]
        if cutz:
            new_arr[i][2] = limit_x_range(
                arr[i][2], box_bound_zmin, box_bound_zmax, box_bound_zl
            )
        else:
            new_arr[i][2] = arr[i][2]
...
def exposed_Xs_Os_boundary_node(
    boundary_node_res, connected_nodeedge_fc, sc_unit_cell, box_bound
):
    ...
    for i in list(boundary_node_res):
        ...
        exposed_Xs_fc = np.asarray(
            [x for x in Xs_fc if not check_nodex_inbox(x.round(4), box_bound)]
        )
        if len(exposed_Xs_fc) > 0:
            ...
            for x in exposed_Xs_cc:
                if check_overlapX(edgex_cvec_array, x):
                    ...
                    node_ovecs_idx = [
                        index
                        for index, value in enumerate(cdist_xos)
                        if value < cdist_xos_sort3rd
                    ]
                    # print(i,n,len(node_ovecs_idx))
                    node_ovecs_cc = [Os_cc[o] for o in node_ovecs_idx]

                    ex_node_cxo_cc_append(
                        (
                            node_center_fc,
                            len(exposed_Xs_cc),
                            "exposed_X",
...
def supercell_nodeedge(supercell_Carte, target_all, sc_unit_cell, box_bound):
    ...
    for i in range(len(supercell_Carte)):
        ...
        info = np.concatenate(
            (fvec_all_info, np.asarray([rescount]).T + i * len(old_res_idx)), axis=1
        )
...
def supercell_nodeedge_fc_loose_check(
    supercell_Carte, target_all_fc, box_bound, scalar, cutx, cuty, cutz
):
    old_res_idx, rescount = fetch_rescount_targetall(target_all_fc)
    fvec_all_info = np.hstack((target_all_fc[:, 0:1], target_all_fc[:, 4:]))
    info = fvec_all_info
    ...
    moded_super_ne_array2d = cut_boundary(
        super_ne_array2d, box_bound, scalar, cutx, cuty, cutz
    )
    row_diff = np.any(super_ne_array2d != moded_super_ne_array2d, axis=1)
    row_diff_idx = [i for i in range(len(row_diff)) if row_diff[i]]
    s_fvec_all = np.hstack((super_ne_info2d, super_ne_array2d))
...
def _filt_boundary_res(s_fvec_all, row_diff_idx, box_bound):
    ...
    for i in res_count_idx:
        if i in differ_res_idx:
            ...
            if (
                res[0, 4] == "EDGE"
                or res[0, 4] == "TEDGE"
                and len(row_diff) > (res.shape[0] - 3)
            ):
                # print(f"see EDGE{i}")
                # check if twoX sit on boundary with a tiny shift range
                if not check_edgex_sits_inboundary(
                    original_fvec[2], original_fvec[5], box_bound
                ):
                    # print(f"kick{i}")
...
def _filt_boundary_res_loose_check(
    s_fvec_all, row_diff_idx, box_bound, scalar, cutx, cuty, cutz, boundary_scalar
):
    res_count_idx = set(s_fvec_all[:, -4])
    differ_res_idx = set([s_fvec_all[i][-4] for i in row_diff_idx])
    # print(f"differ res idx{differ_res_idx}")
    ...
    for i in res_count_idx:
        if i in differ_res_idx:
            ...
            moded_fvec = cut_boundary(
                original_fvec, box_bound, scalar, cutx, cuty, cutz
            )
            ...
            for diff_e in diff_ele:
                if res[0, 4] == "NODE":
                    ...
                    check1 = np.mean(x_atoms[:, -3:], axis=0) + np.asarray(
                        diff_e
                    )  # edge_center
                    # check1=check1.astype(float)
                    # check1 = np.round(check1,1)
                    # print(i,diff_e,check1)
                    if check_edge_center_inbox_loose(
                        check1, box_bound, boundary_scalar
                    ):
                        # print("in",i,diff_e)
                        # print(i,np.round(t_res_xyz[:6],5),np.round(np.mod(t_res_xyz[:6],[1,1,1]),5),diff_e)
                        t_res = np.hstack(
...
def cluster_supercell(
    ...
    s_fvec_all_loose, row_diff_idx_loose = supercell_nodeedge_fc_loose_check(
        supercell_Carte, target_all_fc, box_bound, scalar, cutx, cuty, cutz
    )
    print("row_diff_idx_loose", row_diff_idx_loose)
    safe_res_fc_loose, extra_res_fc_loose, boundary_node_res_loose = (
        _filt_boundary_res_loose_check(
            s_fvec_all_loose,
            row_diff_idx_loose,
            box_bound,
            scalar,
    ...
    connected_nodeedge_fc_loose, boundary_connected_nodes_res, eG = (
        filter_connected_node_loose(
            bare_nodeedge_fc_loose, boundary_node_res_loose, linker_topics, sc_unit_cell
        )
    )
    return (
        connected_nodeedge_fc_loose,
        boundary_connected_nodes_res,
        eG,
        bare_nodeedge_fc_loose,
    )
