...
class prepare():
    ...
    def submit_template(self,template_cif,mof_family,template_mof_node_connectivity,template_node_metal,template_linker_topic):
        ...
        if mof_family in self.mof_top_dict.keys():
            print(f'{mof_family} already exists in the database, the template you submitted will not be used')
            print('if you want to use the template you submitted, please go to the database and replace the existing template')
            return
        else:
            self.mof_top_dict[mof_family] = {'node_connectivity':template_mof_node_connectivity,
            ...
            with open(os.path.join(self.data_path,'MOF_topology_dict'), 'w') as fp:
                ...
                for key in self.mof_top_dict.keys():
                    for met in self.mof_top_dict[key]['metal']:
                        ...
                        line = "{:15s} {:^16d} {:^12s} {:^12d} {:^18s}".format(key,
    ...
    def fetch_node(self):
        ...
        if not hasattr(self,'dummy_node'):
            print('if you want to use dummy node, please select dummy node type with select_dummy_node method')
    ...
    def fetch_linker(self,linker_file):
        ...
        if not hasattr(self,'save_edges_dir'):
            self.save_edges_dir = 'edges' #default  #NOTE: this is the path to the edges
        ...
        if not hasattr(self,'split_linker_topic'):
            ...
            if split_linker_topic != self.linker_topic:
                #warning
                print(f'{split_linker_topic} topic linker is selected for linker splitting')
                print(f'but {self.linker_topic} topic linker is selected for MOF family')
        ...
        (self.linker_center_frag_nodes_num, 
         ...
         self.selected_linker_edge_pdb) = process_linker_molecule(molecule, 
                                                         split_linker_topic, 
