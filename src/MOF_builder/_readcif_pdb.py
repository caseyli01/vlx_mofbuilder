import re
import numpy as np
from _process_cifstr import (
    remove_bracket,
    remove_tail_number,
    remove_quotes,
    extract_quote_lines,
    extract_xyz_lines,
    extract_transformation_matrix_from_symmetry_operator,
)
...
def _extract_atoms_ccoords_from_lines(cell_info, atom_site_sector, f_com):
    ...
    array_xyz = (
        array_xyz - f_com
    )  # make the center of mass(all x) as the origin, in f_coords
...
def apply_sym_operator(symmetry_operations, array_metal_xyz):
    ...
    for sym_line_idx in range(len(symmetry_operations)):
        transfromation_matrix = extract_transformation_matrix_from_symmetry_operator(
        ...
        new_extend_xyz = np.matmul(transfromation_matrix, array_metal_extend_xyz.T).T
        new_xyz = new_extend_xyz[:, 0:3]
        cell_array_metal_xyz = np.vstack((cell_array_metal_xyz, new_xyz))

    round_cell_array_metal_xyz = np.round(limit_value_0_1(cell_array_metal_xyz), 4)
...
def process_node(chain_node_cif, target_type):
    _, _, node_target_atoms = extract_type_atoms_fcoords_in_primitive_cell(
        chain_node_cif, target_type
    )
    _, _, node_x_fcoords = extract_type_atoms_fcoords_in_primitive_cell(
        chain_node_cif, "X"
    )
